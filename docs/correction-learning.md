...
| 공격 | 방어 | 잔여 위험 |
|---|---|---|
| 본문으로 임의 인물에게 태스크 배정 (`"assignee: 관리자"`) | G2: envelope 인물(멘션·발신자·Gmail To/Cc·현재 사용자) 외에는 원문 등장 + contacts 실존 둘 다 필요, 실패 시 `shared` 강등 | contacts에 이미 있는 이름을 본문에 쓰면 통과 — 단 그 인물은 사용자가 등록한 실존 컨택트이므로 권한 상승이 아니라 오배정(가시적, 수정 가능). 또한 모델이 인젝션 지시를 **수신자 자신(self)** 배정으로 해석하면 envelope상 정당하여 통과 — 실측 확인됨. 태스크는 실행되지 않는 읽기 항목이라 스팸 수준이며, 삭제가 suppress 학습으로 흡수 |
