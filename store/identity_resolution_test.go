...
func TestIdentityResolutionViews(t *testing.T) {
	...
	t.Run("v_messages integration", func(t *testing.T) {
		...
		_, err := GetDB().Exec("INSERT INTO messages (user_email, task, source, requester, assignee, source_ts, pinned, done, is_deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
			tenantEmail, "Urgent Task", "whatsapp", childEmail, masterEmail, sourceTS, false, 0, 0)
		if err != nil {
