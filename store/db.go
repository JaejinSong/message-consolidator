...
func runFullDDL(ctx context.Context, dbConn *sql.DB) error {
	...
	if err := addMessagesUpdatedAtColumn(ctx, tx); err != nil {
		return fmt.Errorf("updated_at column migration failed: %w", err)
	}

	if err := addThinkingTokensColumn(ctx, tx); err != nil {
		return fmt.Errorf("thinking_tokens column migration failed: %w", err)
	}

	if err := addCachedTokensColumn(ctx, tx); err != nil {
	...
	if err := backfillZeroTimeAssignedAt(ctx, tx); err != nil {
		return fmt.Errorf("backfill zero-time assigned_at failed: %w", err)
	}

	if err := migrateLifecycleExcluded(ctx, tx); err != nil {
	...
	if err := normalizeJSONDefaults(ctx, tx); err != nil {
