...
# Why: ./... already covers ./tests/regression with the regression tag — running it
# separately just duplicated $GOCACHE lookups and stole CPU from the main test run.
echo -ne "[ RUN ] Go Unit + Regression Tests... \r"
...
print_status $STATUS_GO "Go Unit + Regression Tests"
print_status $STATUS_NPM "NPM (Vitest) Tests"
print_status $STATUS_UI "Loading UI Verification"

# If any failed, show logs
GLOBAL_EXIT=0
if [ $STATUS_GO -ne 0 ] || [ $STATUS_NPM -ne 0 ] || [ $STATUS_UI -ne 0 ]; then
    GLOBAL_EXIT=1
    echo -e "\n${RED}------------------- FAILURE LOGS -------------------${NC}"
    if [ $STATUS_GO -ne 0 ]; then
        echo -e "${RED}Failure in: Go Unit + Regression Tests${NC}"
        cat "$LOG_GO"
    fi
