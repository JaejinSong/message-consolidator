...
- [x] **Anki 스타일 고도화 통계 (Insights)**
  ...
  - [x] **Insights 탭 UI 최적화 및 레이아웃 안정화**
    - [x] 통계(Statistics)와 보고서(Reports) 섹션의 레이아웃 중첩 및 간섭 문제 해결
    - [x] 탭 전환 시스템의 BEM 클래스 기반 가시성 제어 로직 강화
    - [x] 보고서 내 마크다운 테이블 및 링크의 다크모드 가독성(색상, 테두리) 개선
    - [x] 아이콘 렌더링 누락 방지를 위한 Lucide 아이콘 초기화 로직 보완
...
- [x] **품질 검증 자동화 및 AI 안정화**
  - [x] Node.js 기반의 핵심 비즈니스 로직(분류, 필터링) 테스트 케이스 보강
  - [x] Empty State 메시지 및 렌더링 리소스 검증 스크립트 추가
  - [x] **AI 분석 조기 중단(Truncation) 문제 해결**: `MaxOutputTokens` 확장 및 회귀 테스트 케이스 업데이트 완료
