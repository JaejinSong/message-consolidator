# 업데이트 소식 (기술) - v2.4.33 (2026-04-13 15:45 UTC)

- [REFACTOR] 공유 스캐너 유틸리티로 태스크 통합 및 담당자 정규화 로직 일원화
- [REFACTOR] Slack/WhatsApp 메시지 분류 카테고리 및 영속성 로직 표준화
- [FEAT] Slack 메시지 분류 로직 정교화 및 예외 케이스 단위 테스트 추가
- [OPTIMIZE] AI 노이즈 필터링 최적화 및 인메모리 테스트 인프라 전환

---

# 업데이트 소식 (기술) - v2.4.32 (2026-04-13 14:09 UTC)

- [FEAT] AI 기반 Gmail 노이즈 필터링 및 마케팅 메일 감지 로직 구현
- [FEAT] 토큰 사용량 통계 내 필터링된 노이즈 수치 추적 기능 추가
- [OPTIMIZE] WhatsApp 프로필 사진 업데이트 이벤트 처리를 제외하여 부하 최적화
- [REFACTOR] 메시지 저장소 내 내부 트랜잭션 및 상태 동기화 유틸리티 구현
- [REFACTOR] 외부 라이브러리 도입을 통한 Gmail 파싱 엔진 고도화
- [OPTIMIZE] 페이지네이션 및 배칭 적용으로 Gmail CC 복구 성능 개선

---

# 업데이트 소식 (기술) - v2.4.31 (2026-04-12 23:14 UTC)

- [OPTIMIZE] 병렬 처리와 배치 업데이트를 통한 Gmail CC 복구 성능 최적화 및 N+1 쿼리 문제 해결
- [REFACTOR] sqlc 도입을 통해 타입 안전한 쿼리와 DAO 메서드로 데이터베이스 레이어 전환
- [FEAT] 데이터베이스 마이그레이션 시스템 구축 및 테스트 성능 최적화
- [REFACTOR] SQLite 설정을 DSN 파라미터로 통합 및 커넥션 풀 설정 최적화
- [FEAT] AI 컨텍스트 최적화를 위한 시간 기반 메시지 그룹화 기능 구현
- [FIX] Gmail 유닛 테스트 회귀 오류 수정 및 저장소 캐싱 안정성 강화

---

# 업데이트 소식 (기술) - v2.4.30 (2026-04-08 06:39 UTC)

- [FEAT] 멀티 채널 지원을 위한 WhatsApp 비즈니스 API 통합
- [FIX] 서로 다른 시간대 간의 메시지 타임스탬프 동기화 오류 해결
- [PERF] 미디어 비중이 높은 대화 스레드의 이미지 디코딩 및 캐싱 최적화
- [UI] 워크스페이스 보기 간 탐색 시 부드러운 화면 전환 애니메이션 적용

---

# 업데이트 소식 (기술) - v2.4.29 (2026-04-08 06:11 UTC)

- [FEAT] 메시지 분류 및 관리를 위한 폴더 정리 기능 구현
- [FIX] 모바일 입력창에서 키보드가 화면을 가리는 현상 수정
- [PERF] 빈번한 API 호출 최적화를 위한 LRU 캐시 시스템 통합
- [STABILITY] 저대역폭 환경에서의 간헐적인 소켓 연결 끊김 현상 해결

---

# 업데이트 소식 (기술) - v2.4.28 (2026-04-08 04:29 UTC)

- [REFACTOR] 사용자 Upsert 작업 시 원자적 트랜잭션 처리 구현
- [REFACTOR] MIME 디코딩 도입을 통한 이메일 파싱 안정성 개선
- [REFACTOR] 작업 할당 로직 최적화 및 '대기' 카테고리를 '공유' 상태로 전환
...
- [SYS] 내부 릴리스 문서 및 프로젝트 상태 메타데이터 업데이트

---

# 업데이트 소식 (기술) - v2.4.25 (2026-04-07 18:07 UTC)

- [FEAT] 정규화된 관계형 스키마 및 DSU 기반 추적을 포함한 Identity-X 식별 시스템 구축
- [FEAT] 대량 별칭 확인 및 연락처 별칭을 위한 Cache-aside 지원 추가
- [REFACTOR] 멀티 그리드 동기화 및 태스크 노드 관리 개선을 위한 메시지 렌더링 개편
- [REFACTOR] 상태 동기화 최적화를 위해 인터벌 폴링을 재귀적 setTimeout 및 Fetch Lock 구조로 변경
- [OPTIMIZE] 태스크 개수 조회를 단일 DB 쿼리로 통합하여 응답 속도 최적화
- [FIX] MIME 대응 파싱 및 UTF-8 이름 지원을 통한 연락처 추출 로직 강화
- [FIX] 인사이트 보고서 탭 최초 진입 시 발생하는 무한 로딩 스피너 버그 수정

---

# 업데이트 소식 (기술) - v2.4.24 (2026-04-06 13:31 UTC)

- [REFACTOR] 백엔드, 프론트엔드 및 AI 프롬프트 전반에 걸쳐 영어 우선(EN-first) 무결성 체계 적용

---

# 업데이트 소식 (기술) - v2.4.23 (2026-04-06 13:25 UTC)

- [SYS] CI/CD 파이프라인 내 WhaTap 빌더 이미지 통합
- [OPTIMIZE] 빌드 성능 최적화 및 이미지 생성 시간 단축

---

# 업데이트 소식 (기술) - v2.4.22 (2026-04-06 11:45 UTC)

- [SYS] 멀티스테이지 Docker 빌드 패턴 도입 및 배포 스크립트 자동화 연동
- [FEAT] AI 리포트 다국어 생성 기능 추가 및 요약 UI 디자인 개선
- [STABILITY] JSON 추출 로직 안정성 강화 및 리포트 날짜 형식 검증 구현
- [SYS] 사이드카 컨테이너를 활용한 프론트엔드 에셋 분리 및 오케스트레이션 최적화
- [PERF] 작업 병합 처리를 통한 AI 리포트 파이프라인 성능 최적화

---

# 업데이트 소식 (기술) - v2.4.21 (2026-04-06 04:13 UTC)

- [REFACTOR] Docker 빌드 레이어 최적화 및 외부 폰트 의존성을 커스텀 서브셋 CSS로 교체
- [REFACTOR] 모듈 임포트 표준화 및 레거시 JS 파일의 TypeScript 전환 완료
- [FEAT] 태스크 병합 로직 구현 및 병합 시 번역 캐시 무효화 적용
- [FEAT] 리포트 자동 로드 구현 및 코어 데이터베이스 뷰의 시작 시 초기화 마이그레이션
- [FEAT] 리포트 생성 멱등성 보장 및 회귀 테스트 데이터 추가
- [UI] 태스크 작업(삭제, 상태 전환)에 대한 낙관적 UI 업데이트 및 롤백 지원 구현
- [FIX] Docker 빌드 오류 수정 및 기본 언어 설정을 영어로 변경

---

# 업데이트 소식 (기술) - v2.4.20 (2026-04-05 16:41 UTC)

- [FEAT] 롤백을 지원하는 태스크 삭제 및 상태 변경 옵티미스틱 UI 구현
- [FEAT] Docker Compose 및 Caddy 기반의 VPS 배포 자동화 및 빌드 최적화
- [OPTIMIZE] Alias Store의 락(Lock) 해제 전 메타데이터 캐싱을 통한 성능 최적화
- [REFACTOR] 생명주기 관리를 위해 스토어, DB, 캐시 전반에 걸쳐 Context 전파 적용
- [REFACTOR] WhatsApp 메시지 처리 로직 모듈화 및 시스템 메시지 필터링 강화
- [REFACTOR] ioutil의 os 패키지 이관 및 미사용 서비스 정리를 통한 코드 현대화
