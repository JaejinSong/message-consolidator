...
## ⚙️ 시스템 및 인프라
- **[FIX] 컨테이너 내 로컬라이징 문서 포함**: `Dockerfile` 및 `.dockerignore`를 수정하여 모든 다국어 릴리즈 노트 파일이 빌드 컨텍스트에 포함되도록 개선했습니다.
- **[SYS] Turso(libsql) DB 마이그레이션**: 글로벌 사용자의 지연 시간 단축을 위해 데이터베이스를 Turso(libsql) 엣지 환경으로 완전히 이관했습니다.
...
## 🐞 버그 수정 및 안정성
- **[FIX] Gmail 담당자 렌더링 오류 수정**: Gmail에서 연동된 업무의 담당자 필드가 `undefined`로 표시되던 UI 버그(v2.3.4 관련)를 수정했습니다. `resolveActualAssignee` 함수의 컨텍스트 파라미터 전달 로직을 정상화했습니다.
- **[STABILITY] 배포 전 자동 검증 강화**: `deploy.sh` 스크립트를 업데이트하여 배포 프로세스에 `npm test` 단계를 필수화했습니다. 이를 통해 모든 테스트를 통과한 안정적인 빌드만 운영 환경에 반영됩니다.

## 🛠️ 내부 리팩토링
- **[SYS] 유틸리티 표준화 완료**: 레거시 내부 도구들을 `mc-util` 구조로 통합 완료하여, 백엔드 서비스와 프론트엔드 로직 간의 결합도를 낮추고 유지보수성을 향상시켰습니다.
...
## 🚀 AI 및 데이터 인텔리전스
- **[FIX] AI 분석 결과 잘림 현상 해결**: 분석 내용이 중간에 끊기던 문제를 수정했습니다. Gemini 클라이언트의 응답 처리 로직을 강화하여 긴 분석 결과도 안정적으로 수신합니다.
- **[REFACTOR] Gemini 클라이언트 최적화**: 토큰 사용량 로깅 및 응답 파싱 로직을 분리하여 AI 운영 비용 추적과 디버깅 효율을 높였습니다.
- **[FEAT] 인사이트 활동 차트**: 사용자 업무 패턴을 한눈에 파악할 수 있도록 Anki 스타일의 시간별 활동 히트맵을 인사이트 UI에 도입했습니다.

## 🛠️ 시스템 아키텍처 및 관제
- **[FEAT] WhaTap 모니터링 도입**: WhaTap 관제 시스템을 통합하여 실시간 애플리케이션 성능(APM) 모니터링 및 세션 리플레이 기능을 강화했습니다.
- **[REFACTOR] SQL View 기반 리팩토링**: 복잡한 쿼리를 데이터베이스 뷰(View)로 전환하여 애플리케이션 계층의 복잡도를 낮추고 쿼리 실행 성능을 최적화했습니다.
- **[SYS] 유틸리티 도구 통합**: 산재해 있던 디버그 및 유틸리티 도구들을 `mc-util` 패키지로 통합하여 백엔드 로직의 표준화를 완료했습니다.

## ⚡ 성능 및 프론트엔드
- **[PERF] Nginx 서버 최적화**: HTTP/2 프로토콜과 Gzip 압축을 활성화하여 리소스 로딩 속도를 높이고 데이터 전송량을 대폭 줄였습니다.
- **[OPTIMIZE] 모바일 UI 개선**: 모바일 화면에서의 여백과 레이아웃을 정교하게 다듬어 다양한 기기에서 쾌적한 대시보드 경험을 제공합니다.
- **[STABILITY] 회귀 테스트 강화**: AI 분석의 일관성과 모바일 UI 무결성을 검증하기 위한 자동화 테스트 케이스를 추가했습니다.
...
## 🚀 데이터베이스 및 핵심 인프라
- **[FEAT] Turso (libsql) 마이그레이션**: 엣지 환경에서의 성능 극대화를 위해 데이터베이스를 Turso로 이전했습니다. 드라이버 초기화 시 발생하던 서버 구동 중단 현상을 완벽히 해결했습니다.
- **[FEAT] 배치 번역 엔진 도입**: 대량의 메시지 처리 시 청크(Chunk) 단위로 나누어 번역하는 로직을 구현했습니다. 이를 통해 커넥션 풀 고갈을 방지하고 번역 효율을 높였습니다.
- **[SYS] 배포 전 자동 검증 강화**: CI/CD 파이프라인의 `deploy.sh` 설정을 업데이트하여 배포 전 `npm test` 통과를 의무화했습니다. 이제 테스트가 실패할 경우 배포가 자동으로 중단되어 안정성을 보장합니다.

## 🎨 대시보드 및 사용자 인터페이스
- **[UI] 대시보드 'All Clear' 로직 개선**: 할 일이 없는 상태의 UI 로직과 디자인을 정교하게 다듬었습니다. 모든 업무가 완료되었을 때 더욱 직관적이고 성취감을 주는 화면을 제공합니다.
- **[FEAT] 업적 및 상태 표시 시스템**: 업적 UI의 시각적 요소를 개선하고 다국어 지원을 강화했습니다. 시간 가독성을 높이고 상태 아이콘을 직관적으로 변경하여 업무 흐름을 한눈에 파악할 수 있게 했습니다.
- **[UI] 알림 메시지(Toast) 도입**: 프론트엔드 전반에 알림 시스템을 도입하여, 백그라운드 작업 상태나 오류 발생 시 실시간으로 피드백을 제공합니다.

## 🐞 버그 수정 및 안정화
- **[FIX] Gmail 담당자 표시 오류 해결**: Gmail에서 가져온 업무의 담당자가 'undefined'로 표시되던 렌더링 버그를 수정했습니다. 컨텍스트 파라미터를 통해 담당자 정보를 정확히 찾아내도록 로직을 보완했습니다.
...
## 🛠️ 시스템 아키텍처 및 관측성 강화
- **[REFACTOR] 유틸리티 라이브러리 통합**: 흩어져 있던 디버깅 및 유틸리티 도구들을 `mc-util`로 통합했습니다. 모든 CI/CD 워크플로우가 이 중앙 집중화된 라이브러리를 사용하도록 업데이트하여 의존성 관리를 최적화했습니다.
- **[SYS] WhaTap 모니터링 도입**: 실시간 성능 관리를 위해 WhaTap APM을 연동했습니다. 이제 서버 측의 병목 현상을 실시간으로 감지하고 문제 발생 시 더욱 신속하게 대응할 수 있습니다.
- **[FIX] AI 분석 결과 중단 현상 해결**: Gemini 분석 엔진의 `MaxOutputTokens` 설정을 4096으로 상향 조정했습니다. 이를 통해 복잡한 메시지 추출 시 응답이 도중에 끊겨 업무 목록이 누락되던 현상을 원천적으로 해결했습니다.
- **[REFACTOR] SQL View 아키텍처 도입**: 복잡한 데이터 조회 로직을 SQL View로 재설계했습니다. 여러 테이블의 조인 과정을 추상화하여 백엔드 코드를 단순화하고 쿼리 유지보수성을 높였습니다.
- **[PERF] 커넥션 풀 최적화**: 데이터베이스 커넥션 풀 설정을 세밀하게 조정하여, 많은 양의 메시지가 동시에 처리되는 환경에서도 자원 고갈 없이 안정적으로 작동하도록 개선했습니다.
- **[OPTIMIZE] 코드베이스 정돈**: 더 이상 사용하지 않는 CSS 클래스와 모듈 폴더를 삭제했습니다. 프론트엔드 빌드 용량을 줄이고 전체적인 시스템 가독성을 향상했습니다.
...
## 📊 데이터 시각화 및 성능 최적화
- **[FEAT] Anki 스타일 활동 히트맵**: Anki의 학습 그리드에서 영감을 받은 시간별 활동 차트를 도입했습니다. 이제 인사이트 UI에서 24시간 동안의 업무 밀도를 한눈에 파악할 수 있습니다.
- **[PERF] Nginx 네트워크 최적화**: 리버스 프록시 설정에 HTTP/2 및 Gzip 압축을 적용했습니다. 데이터 전송 효율을 높여 특히 모바일 환경에서의 초기 로딩 속도를 대폭 개선했습니다.
- **[REFACTOR] Gemini 클라이언트 고도화**: AI 클라이언트의 토큰 사용량 로깅과 응답 처리 로직을 추상화했습니다. 코드 가독성을 높이고 향후 다양한 AI 모델 도입을 위한 기반을 마련했습니다.
- **[FIX] Gmail 담당자 표시 오류**: Gmail에서 연동된 작업의 담당자가 'undefined'로 렌더링되던 버그를 해결했습니다.
- **[STABILITY] 배포 검증 프로세스 강화**: `deploy.sh` 스크립트에 `npm test` 단계를 필수 과정으로 추가했습니다. 이제 테스트를 통과하지 못한 코드는 배포되지 않도록 자동 방어 로직이 작동합니다.
- **[UI] 대시보드 상태 로직 정교화**: 모든 채널의 메시지가 정리되었을 때 표시되는 'All Clear' 상태 로직을 개선하여 데이터 정밀도를 높였습니다.
...
- **[FEAT] Turso(libsql) DB 마이그레이션**: 메인 데이터베이스를 Turso 아키텍처로 이전했습니다. 이를 통해 엣지 환경에서의 성능을 극대화하고, 간헐적으로 발생하던 서버 시작 시점의 패닉 현상을 완벽히 해결했습니다.
- **[FEAT] 번역 엔진 배치 청킹**: 대량의 메시지 번역 시 발생할 수 있는 타임아웃을 방지하기 위해 데이터를 적절한 단위로 나누어 처리하는 청킹(Chunking) 로직을 도입했습니다.
- **[OPTIMIZE] 커넥션 풀링 최적화**: 데이터베이스 연결 관리 로직을 고도화하여, 동시 접속자가 많은 상황에서도 지연 시간(Latency)을 최소화했습니다.
- **[FEAT] 글로벌 타임존 대응**: 모든 시간 데이터를 `TIMESTAMPTZ` 형식으로 전환했습니다. 이제 전 세계 어디서 접속하더라도 사용자 지역에 맞는 정확한 통계와 활동 내역을 확인할 수 있습니다.
- **[UI] 실시간 토스트 알림**: 시스템의 상태 변화나 작업 완료 여부를 즉각적으로 알 수 있도록 세련된 토스트 알림 시스템을 추가했습니다.
...
- **[FEAT] WhaTap 모니터링 도입**: 실시간 서버 상태 감시와 성능 추적을 위해 WhaTap 연동을 완료했습니다. 세션 리플레이 로직 최적화도 함께 진행되었습니다.
- **[REFACTOR] SQL View 기반 아키텍처**: 복잡한 쿼리 로직을 데이터베이스 View로 이관하여 백엔드 코드의 복잡도를 낮추고 데이터 처리 효율을 개선했습니다.
...
- **[SYS] 배포 검증 강화**: `deploy.sh` 스크립트에 `npm test` 단계를 필수 과정으로 추가하여, 테스트를 통과하지 않은 빌드가 배포되지 않도록 방어 로직을 강화했습니다.
...
## 📊 시각적 분석 강화 및 성능 최적화
- **[FEAT] Anki 스타일 활동 차트**: 인사이트 대시보드에 시간대별 활동 히트맵을 도입했습니다. Anki 스타일의 그리드를 통해 메시지 통합 패턴을 직관적으로 파악할 수 있습니다.
- **[OPTIMIZE] 네트워크 전송 최적화**: Nginx 설정에 HTTP/2 및 Gzip 압축을 적용했습니다. 이를 통해 리소스 로딩 속도를 개선하고 초기 응답 시간(TTFB)을 대폭 단축했습니다.
