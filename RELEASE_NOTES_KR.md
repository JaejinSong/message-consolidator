# Release Notes (기술 버전 - 국문) - v2.1.5 (2026-03-21 13:10 UTC)

## 🏗️ 인사이트 및 게이미피케이션을 위한 아키텍처 리팩토링
- **[REFACTOR] 백엔드 서비스 레이어 도입**: XP, 스트릭, 업무 완료 로직을 데이터 접근 레이어(`store`) 및 핸들러에서 분리하여 `services` 패키지로 구조화했습니다.
- **[REFACTOR] 프론트엔드 이벤트 기반 아키텍처 전환**: 중앙 이벤트 버스(`events.js`)를 구현하여 핵심 동작과 UI 부수 효과(연출, 통계 갱신 등)를 완전히 분리했습니다.
- **[OPTIMIZE] 렌더러 로직 모듈화**: 사용자 경험치 계산 및 프로필 업데이트 로직을 유틸리티화하여 향후 '인사이트' 탭 구현을 위한 기반을 마련했습니다.
